...
from intel.concourse.models.concourse_model import ConcoursePipeline, ConcourseTeam, ConcoursePipelineGroup, \
    ConcourseSecret, ConcourseJob, ConcoursePlan
...
class DiscPipelines(ConcourseDisc):
    ...
    def _disc_pipelines(self, pipeline, **kwargs):
        ...
        pipe_info = self.call_concourse_api(f=self.cred.get_config, ret_val={},
                                            **{"team_name": team_name, "pipeline_name": name})
            ...
            for i, plan in enumerate(job["plan"]):
                ...
                if config.get("image_resource"):
                    ...
                    if image_resource.get("type") == "docker-image":
                        if repo := image_resource.get("source", {}).get(
                            "repository"
                        ):
