...
class DiscStorage(GcpDisc):
    ...
    def _disc_storages(self, p_obj: GcpProject):
        ...
        storages: List[dict] = self.execute_http_req(http_prep, "items", disable_warn=True,
                                                     list_kwargs={"project": project_name.split("/")[1]})
        for storage in storages:
            storage_obj: GcpStorage = GcpStorage(
                ...
                publicAccessPrevention=storage["iamConfiguration"].get("publicAccessPrevention",
                                                                       "") if "iamConfiguration" in storage else "",
                bucketPolicyOnly=storage["iamConfiguration"]["bucketPolicyOnly"].get("enabled",
                                                                                     False) if "iamConfiguration" in storage and "bucketPolicyOnly" in
                                                                                               storage[
                                                                                                   "iamConfiguration"] else False,
                uniformBucketLevelAccess=storage["iamConfiguration"]["uniformBucketLevelAccess"].get("enabled",
                                                                                                     False) if "iamConfiguration" in storage and "uniformBucketLevelAccess" in
                                                                                                               storage[
                                                                                                                   "iamConfiguration"] else False,
            ).save()
    ...
    def _disc_files(self, p_obj: GcpProject, storage_obj: GcpStorage):
        ...
        files: List[dict] = self.execute_http_req(http_prep, "items", disable_warn=True,
                                                  list_kwargs={"bucket": storage_obj.name})
        for file in files:
            file_obj = GcpFile(
                name=file["id"],
