...
class AnalyzeResults(GcpDisc):
    ...
    def _disc(self) -> None:
        ...
        with open(f"{os.path.dirname(__file__)}/../info/privesc.yaml", "r") as stream:
    ...
    def _disc_privesc_technique(self, prives_def):
        ...
        if extra_privesc_to == "RUNNING_SA" and not running_in:
            self.logger.error(
                f"The privecs technique {title} is of type RUNNING_SA but no class is indicated in 'running_in'. This won't be checked.")
            return
        ...
        for role in roles:
            ...
            for ppal_rsc in ppals_rscs:
                ...
                for rsc_reasons in resources_and_reasons:
                    ...
                    resources_to_relate: List[GcpResource] = [
                        res] if not second_order_permissions else []  # If second_order_permissions, start empty
                    ...
                    if can_escalate is None:
                        can_escalate, more_reasons, _ = self._has_other_perms_to_escalate(permissions, projectNumber,
                                                                                          ppal, summary,
                                                                                          only_to_classes,
                                                                                          extra_privesc_to, running_in,
                                                                                          is_second_order_relations=bool(
                                                                                              second_order_relations))
                    ...
                    if second_order_permissions:
                        if res.__primaryvalue__ not in self.known_2order_res:
                            self.known_2order_res[res.__primaryvalue__] = self._has_other_perms_to_escalate(
                                second_order_permissions,
                                projectNumber, res, summary, only_to_classes, extra_privesc_to, running_in,
                                is_second_order_perms=True)
                    ...
                    if can_escalate and resources_to_relate:
                        ...
                        for rsc in resources_to_relate:
                            ...
                            if rsc.__node__.has_label("Gcp") and not rsc.__node__.has_label("GcpResource"):
                                self.logger.debug(
                                    f"Resource {res.__primaryvalue__} of type {type(res)} is not a GcpResource, so it's not going to be related")
                                continue

                            # Check if the final class is allowed
                            if only_to_classes and all(
                                f".{class_name}" not in str(type(rsc))
                                for class_name in only_to_classes
                            ):
                                continue
                            ...
                            ppal = ppal.privesc_to(rsc, reasons=reasons, title=title, summary=summary,
                                                   limitations=limitations)

    def _update_interesting_permissions(self, ppal: GcpPrincipal, role: str, permission: str, res: GcpResource,
                                        summary: str, second_order=False):
        ...
        if not ppal.interesting_permissions:
            ...
            ppal.save()
        elif interesting_perm not in ppal.interesting_permissions:
    ...
    def _get_roles_with_permission(self, perm_name: str) -> List[GcpRole]:
        ...
        if "*" not in perm_name:
            return (
                [p[0] for p in perm.roles._related_objects]
                if (perm := GcpPermission.get_by_name(perm_name))
                else []
            )
        ...
        for perm in perms:
            for role, _ in perm.roles._related_objects:
                if role.__primaryvalue__ not in known_roles:
                    roles.append(role)
    ...
    def _get_project_number_from_res(self, res: GcpResource):
        ...
        if hasattr(res, "projectNumber"):
            ...
            if len(related) > 0 and len(related[0]) > 0 and hasattr(related[0][0], "projectNumber"):
    ...
    def _get_principals_with_role(self, role: str, only_to_classes: List[str], extra_privesc_to: str,
                                  running_in: list, second_order_relations: List[str]) -> List[
        Tuple[GcpPrincipal, List[Tuple[GcpResource, str]]]]:
        """
        ...
        for ppal in principals_with_role:
            resources = self._get_assets_from_principal_with_role(ppal, role, only_to_classes, extra_privesc_to,
                                                                  running_in, second_order_relations)
    ...
    def _has_other_perms_to_escalate(self, permissions, projectNumber, ppal, summary, only_to_classes: List[str],
                                     extra_privesc_to: str, running_in: list, is_second_order_perms=False,
                                     is_second_order_relations=False):
        """
        ...
        for perm in permissions:
            ...
            for role in roles:
                can_escalate = False
                rsc_reasons2 = self._get_assets_from_principal_with_role(ppal, role, only_to_classes, extra_privesc_to,
                                                                         running_in, second_order_relations=[])
    ...
    def _get_assets_from_principal_with_role(self, ppal: GcpPrincipal, role: str, only_to_classes: List[str],
                                             extra_privesc_to: str,
                                             running_in: list, second_order_relations: List[str]) -> List[
        Tuple[GcpResource, str]]:
        """
        ...
        for resource, roles in ppal.has_perm._related_objects:
            if role in roles["roles"]:
                ...
                resources += self._get_recursive_resources(resource, resources_already, only_to_classes,
                                                           extra_privesc_to, role, running_in, second_order_relations,
                                                           from_obj=[])
    ...
    def _get_recursive_resources(self, res, resources_already, only_to_classes, extra_privesc_to,
        ...
        elif only_to_classes and res_class_name not in only_to_classes:
            # If class not authorized, get the authorized object
            from_obj += [res]
            res = self._check_privesc_to_res(res, extra_privesc_to)  # returned res COULD be a list!
        ...
        if type(res) is list:
            for r in res:
                ...
                more_resources += self._get_recursive_resources(r, resources_already, only_to_classes,
                                                                extra_privesc_to, role, running_in,
                                                                second_order_relations=[], from_obj=from_obj)
            ...
            if from_obj:
                more_resources.append(
                    (
                        res,
                        f"Indirect role {role} from {' -> '.join([f'{f.__class__.__name__}({f.__primaryvalue__})' for f in from_obj])}",
                    )
                )
            else:
            ...
            for scope in self.analysis_data["scope"]:
                ...
                if res.__class__.__name__ == class_name:
                    ...
                    privesc_to_types = list(set([s["initial_class_name"] for s in self.analysis_data["scope"]] + (
                        running_in or only_to_classes)))
                    cypher_labels = '"' + '", "'.join(privesc_to_types) + '"'
                    relation_name = scope["relation"]
                    objs = res.get_by_relation(relation_name,
                                               where=f"EXISTS ((a)<-[:{relation_name}]-(b)) AND any(l in labels(b) WHERE l in [{cypher_labels}])")

                    for obj in objs:
                        more_resources += self._get_recursive_resources(obj, resources_already, only_to_classes,
                                                                        extra_privesc_to,
                                                                        role, running_in, second_order_relations,
                                                                        from_obj=from_obj)
    ...
    def _check_privesc_to_res(self, res: CustomOGM, extra_privesc_to):
        ...
        if extra_privesc_to == "ALL_PROJECT_SAS":
            ...
            for k8s_obj, _ in res.k8s_namespaces._related_objects:
                ...
                obj.extend(
                    iter(
                        K8sServiceAccount.get_all_by_kwargs(
                            f'_.name =~ "^{k8s_obj.name}:.*"'
                        )
                    )
                )
        return obj
    ...
    def _get_second_relations(self, rscs: List[CustomOGM], sor: List[str]) -> List[CustomOGM]:
        ...
        for rsc in rscs:
            for sec_rsc in rsc.get_by_relation(rel_name):
                ...
                if cache_name not in known_cache:
        ...
        return self._get_second_relations(sec_rscs, sor) if sec_rscs else []
