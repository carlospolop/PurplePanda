...
class DiscCloudRun(GcpDisc):
    ...
    def _disc_cloudrun(self, p_obj:GcpProject):
        ...
        for loc in locations:
            ...
            run_services: List[str] = self.execute_http_req(http_prep, "items", disable_warn=True, list_kwargs={"parent": parent_cf_fullname})
            ...
            for run_svc in run_services:
                ...
                ports = [p["containerPort"] for c in containers_info if c.get("ports") for p in c["ports"] if p.get("containerPort")]
                status = run_svc["status"]

                run_obj = GcpCloudRunSvc(
                    name = parent_cf_fullname + f"/services/{metadata['name']}",
                    displayName = metadata["name"],
                    namespace = metadata["namespace"],
                    selfLink = metadata["selfLink"],
                    uid = metadata["uid"],
                    creator = annotations.get("serving.knative.dev/creator", ""),
                    lastModifier = annotations.get("serving.knative.dev/lastModifier", ""),
                    user_image = annotations.get("client.knative.dev/user-image", ""),
                ...
                sa_email = spec.get("serviceAccountName", f"{p_obj.projectNumber}-compute@developer.gserviceaccount.com")
