...
class DiscComputeInstances(GcpDisc):
    ...
    def _disc_instances(self, p_obj: GcpProject):
        ...
        locations: List[str] = self.execute_http_req(http_prep, "items", disable_warn=True,
        ...
        for loc in locations:
            ...
            instances: List[str] = self.execute_http_req(http_prep, "items", disable_warn=True,
                                                         list_kwargs={"project": project_id})
            ...
            for inst in instances:
                inst_obj = GcpComputeInstance(
                    ...
                    enableSecureBoot=inst["shieldedInstanceConfig"].get("enableSecureBoot",
                                                                        False) if "shieldedInstanceConfig" in inst else False,
                    enableVtpm=inst["shieldedInstanceConfig"].get("enableVtpm", False) if "enableVtpm" in inst else "",
                    enableIntegrityMonitoring=inst["shieldedInstanceConfig"].get("enableIntegrityMonitoring",
                                                                                 False) if "enableIntegrityMonitoring" in inst else False,
                    updateAutoLearnPolicy=inst["shieldedInstanceIntegrityPolicy"].get("updateAutoLearnPolicy",
                                                                                      False) if "shieldedInstanceIntegrityPolicy" in inst else False,
                ...
                self.get_iam_policy(inst_obj, self.service.instances(), inst_obj.name, project=project_id,
                                    zone=location)
                ...
                if inst.get("disks"):
                    for disk in inst.get("disks"):
                        ...
                        self.get_iam_policy(disk_obj, self.service.disks(), disk_obj.name, project=project_id,
                                            zone=location)
                ...
                if inst.get("networkInterfaces"):
                    for nic in inst.get("networkInterfaces"):
                        ...
                        natIps += [
                            elem["externalIpv6"]
                            for elem in accessConfigs
                            if "externalIpv6" in elem
                            and elem["externalIpv6"] not in natIps
                        ]
                        ...
                        natIpsv6 += [
                            ...
                            for elem in accessConfigs
                            if "externalIpv6" in elem
                            and elem["externalIpv6"] not in natIpsv6
                        ]
