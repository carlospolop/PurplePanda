...
class DiscComputeSubnetworks(GcpDisc):
    ...
    def _disc_subnetworks(self, p_obj: GcpProject):
        ...
        loc_names = {"-".join(l["name"].split("-")[:-1]) for l in locations}

        for location in loc_names:
            http_prep = self.service.subnetworks()  # .list(project=project_id, region=location)
            subnetworks: List[str] = self.execute_http_req(http_prep, "items", disable_warn=True,
                                                           list_kwargs={"project": project_id, "region": location})
            ...
            for subnet in subnetworks:
                ...
                self.get_iam_policy(subnet_obj, self.service.subnetworks(), subnet_obj.name, project=project_id,
                                    region=location)
