...
class DiscKMS(GcpDisc):
    ...
    def _disc_keyrings(self, p_obj:GcpProject):
        ...
        for loc in locations:
            ...
            keyrings: List[str] = self.execute_http_req(http_prep, "keyRings", disable_warn=True, list_kwargs={"parent": parent_fullname})
    ...
    def _disc_keys(self, p_obj:GcpProject, keyring_obj: GcpKMSKeyRing):
        ...
        cryptokeys: List[str] = self.execute_http_req(http_prep, "cryptoKeys", disable_warn=True, list_kwargs={"parent": keyring_obj.name})
        for cryptokey in cryptokeys:
            ...
            self.get_iam_policy(cryptokey_obj, self.service.projects().locations().keyRings().cryptoKeys(), cryptokey_obj.name)
