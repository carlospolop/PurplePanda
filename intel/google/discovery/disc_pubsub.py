...
class DiscPubSub(GcpDisc):
    ...
    def _disc_topics(self, p_obj: GcpProject):
        """Discover all the pubsubs topics of a project"""

        project_name: str = p_obj.name
        http_prep = self.service.projects().topics()  # .list(project=project_name)
        topics: List[str] = self.execute_http_req(http_prep, "topics", disable_warn=True,
                                                  list_kwargs={"project": project_name})
        for topic in topics:
    ...
    def _disc_pubsub_subscriptions(self, p_obj: GcpProject):
        """Discover all the pubsubs subscriptions of a project"""

        project_name: str = p_obj.name
        http_prep = self.service.projects().subscriptions()  # .list(project=project_name)
        subscriptions: List[str] = self.execute_http_req(http_prep, "subscriptions", disable_warn=True,
                                                         list_kwargs={"project": project_name})
        for sub in subscriptions:
            pushConfig = sub.get("pushConfig", {})
            ...
            self.get_iam_policy(sub_obj, self.service.projects().subscriptions(), sub_obj.name)

    def _disc_pubsub_schemas(self, p_obj: GcpProject):
        """Discover all the pubsubs schemas of a project"""

        project_name: str = p_obj.name
        http_prep = self.service.projects().schemas()  # .list(parent=project_name)
        schemas: List[str] = self.execute_http_req(http_prep, "schemas", disable_warn=True,
                                                   list_kwargs={"parent": project_name})
