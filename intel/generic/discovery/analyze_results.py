...
class AnalyzeResults(PurplePanda):
    ...
    def _disc(self, flags) -> None:
        ...
        self._disc_loop([None], self._merge_k8s_sas_with_unknwon_cluster_id,
                        __name__.split(".")[-1] + "._merge_k8s_sas_with_unknwon_cluster_id")

        self._disc_loop([None], self._merge_concourse_workers_with_pods,
                        __name__.split(".")[-1] + "._merge_concourse_workers_with_pods")

        self._disc_loop([None], self._merge_github_writers_steal_circleci_secrets,
                        __name__.split(".")[-1] + "._merge_github_writers_steal_circleci_secrets")
    ...
    def _merge_k8s_sas_with_unknwon_cluster_id(self, _):
        ...
        query = (
            'MATCH (ksa:K8sServiceAccount)-[r:PRIVESC]->(b)\n'
            + 'MATCH (ksa2:K8sServiceAccount) WHERE ksa2.name =~ ".+-"+ksa.name\n'
        )
    ...
    def _merge_concourse_workers_with_pods(self, _):
        ...
        query = (
            'MATCH(worker:ConcourseWorker)\n'
            + 'MATCH(pod:K8sPod) WHERE pod.name =~ ".+"+worker.name\n'
        )
    ...
    def _merge_github_writers_steal_circleci_secrets(self, _):
        ...
        query = (
            'MATCH(ppal:Github)-[perms:HAS_PERMS]->(repo:GithubRepo)-[r_circle:IN_CIRCLECI]->(project:CircleCIProject)-[r_secrets:HAS_SECRET]->(secret:CircleCISecret)\n'
            + 'WHERE perms.admin = True OR perms.maintain = True OR perms.push = True\n'
        )
        ...
        query = (
            'MATCH (org)<-[:PART_OF]-(:CircleCIContext)-[:HAS_SECRET]->(secret:CircleCISecret)\n'
            + 'WITH org,  COLLECT (DISTINCT secret) as s\n'
        )
    ...
    def _create_same_emails_rels(self, _):
        ...
        query = 'MATCH (user1), (user2)' + 'WHERE ID(user1) <> ID(user2) AND ('
