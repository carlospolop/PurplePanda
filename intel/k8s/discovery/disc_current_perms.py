...
class DiscCurrentPerms(K8sDisc):
    ...
    def _disc(self) -> None:
        ...
        self._disc_loop(namespaces, self._disc_current_perms, __name__.split(".")[-1], **{"client_cred": client_cred, "user_obj": user_obj})
    ...
    def _disc_current_perms(self, ns_obj:K8sNamespace, **kwargs):
        ...
        for perm in current_perms.status.resource_rules:
            for res in perm.resources:
                ...
                res_obj.users.update(user_obj, role_name="unknown", bind_name="unknown", kind_bind="unknown", verbs=perm.verbs, apiGroups=perm.api_groups)
                res_obj.save()
