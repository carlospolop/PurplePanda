...
class DiscReplicationControllers(K8sDisc):
    ...
    def _disc_replicationcontroller(self, ns_obj: K8sNamespace, **kwargs):
        ...
        replicationcontroller = self.call_k8s_api(f=client_cred.list_namespaced_replication_controller,
                                                  namespace=ns_obj.ns_name)
        if not replicationcontroller or not replicationcontroller.items:
            return

        self._disc_loop(replicationcontroller.items, self._save_replicationcontroller,
                        __name__.split(".")[-1] + f"-{ns_obj.ns_name}", **{"orig": ns_obj})
