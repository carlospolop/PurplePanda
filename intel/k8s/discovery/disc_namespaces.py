...
class DiscNamespaces(K8sDisc):
    ...
    def _disc_nss(self, ns):
        ns_obj = K8sNamespace(
            ...
            iam_amazonaws_com_permitted=ns.metadata.annotations.get("iam.amazonaws.com/permitted",
                                                                    "") if ns.metadata.annotations else "",
            iam_amazonaws_com_allowed_roles=ns.metadata.annotations.get("iam.amazonaws.com/allowed-roles",
                                                                        "") if ns.metadata.annotations else "",
        ).save()
