...
class DiscServiceAccounts(K8sDisc):
    ...
    def _disc(self) -> None:
        ...
        self._disc_loop(namespaces, self._disc_sas, __name__.split(".")[-1])
    ...
    def _disc_sas(self, ns_obj: K8sNamespace, **kwargs):
        ...
        self._disc_loop(sas.items, self._save_sa, __name__.split(".")[-1] + f"-{ns_obj.ns_name}", **{"ns_obj": ns_obj})

    def _save_sa(self, sa, **kwargs):
        """Given a sa and context, save it"""
        ...
        sa_obj = K8sServiceAccount(
            ...
            iam_amazonaws_role_arn=sa.metadata.annotations.get("eks.amazonaws.com/role-arn",
                                                               "") if sa.metadata.annotations else "",
        ...
        if sa.metadata.annotations and "iam.gke.io/gcp-service-account" in sa.metadata.annotations.keys():
            ...
            sa_obj = sa_obj.privesc_to(gcpsa_obj, reasons=reasons, title=title, summary=summary,
