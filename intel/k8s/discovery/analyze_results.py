...
class AnalyzeResults(K8sDisc):
    ...
    def _create_known_groups_relations(self):
        ...
        query = """MERGE (auth_group:K8s:K8sPrincipal:K8sGroup{name:"system:authenticated"})
                WITH auth_group
                MATCH (users:K8sUser)
                MERGE (users)-[:MEMBER_OF]->(auth_group)
                WITH auth_group
                MATCH (groups:K8sGroup) WHERE groups.name <> "system:authenticated"
        ...
        query = """MERGE (group:K8s:K8sPrincipal:K8sGroup{name:"system:serviceaccounts"})
                WITH group
        ...
        query = """MATCH (ns:K8sNamespace)<-[:PART_OF]-(sa:K8sServiceAccount)
                MERGE (group:K8s:K8sPrincipal:K8sGroup{name:"system:serviceaccounts:"+ns.name})
