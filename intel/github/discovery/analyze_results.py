...
from intel.github.models.github_model import GithubBranch, GithubOrganization, GithubRepo, GithubSecret, GithubUser, \
    GithubTeam
...
class AnalyzeResults(GithubDisc):
    ...
    def _disc(self) -> None:
        ...
        self._disc_loop(GithubOrganization.get_all(), self._set_repo_admins,
                        __name__.split(".")[-1] + "._set_repo_admins")

        # Roles write, maintain and admin --> steal repo secrets and org secrets
        self._disc_loop(GithubOrganization.get_all(), self._steal_repo_secrets,
                        __name__.split(".")[-1] + "._steal_repo_secrets")

        # orgs.members_can_create_repositories --> Any user can steal org secrets
        self._disc_loop(GithubOrganization.get_all(), self._steal_org_secrets_with_generic_write_permission,
                        __name__.split(".")[-1] + "._steal_org_secrets_with_generic_write_permission")

        # Find users that can write in repos with self-hosted runners
        self._disc_loop(GithubOrganization.get_all(), self._run_selfhosted_runner,
                        __name__.split(".")[-1] + "._run_selfhosted_runner")

        # Find users that can write to the default branch
        self._disc_loop(GithubOrganization.get_all(), self._write_to_default_branch,
                        __name__.split(".")[-1] + "._write_to_default_branch")
                    ...
                    if branch_obj.name == default_branch_name:
                        ...
                        if not known_protections and not self.printed_no_protections_msg:
                            ...
                            if not self.github_write_as_merge:
                                self.logger.error(
                                    "I couldn't get branch protections of all or some repos so False Positives and False Negatives can occur. I will try to minimize them... To treat write as merge permissions use the param '--github-write-as-merge'")
                            else:
                                self.logger.error(
                                    "I couldn't get branch protections of all or some repos so False Positives and False Negatives can occur. I will treat all write as merge permission as indicated.")
                                self._relate_to_merge(branch_obj, write_ppals,
                                                      reason="User indicated to treat all writes as merge")
                                continue
                        ...
                        # TODO: We are only getting if everyone is allowed. Get if specific people/groups are allowed
                                    ...
                                    if type(codeowner) is GithubUser:
                                        ...
                                        for user_co, _ in codeowner.users._related_objects:
                                            if not type(user_co) is GithubUser:
                                                self.logger.error(
                                                    "Something that isn't a GithubUser is related to a GithubTeam")
                                            else:
                                                codeowners.append(user_co)

                                    else:
                                        self.logger.error(
                                            "Something that isn't a GithubUser or GithubTeam is a codeowner")
                            ...
                            if str(branch_obj.required_approving_review_count) == "1" or not known_protections:
                                ...
                                if (branch_obj.require_code_owner_reviews or not known_protections) and (
                                        repo_obj.unkown_codeowners or repo_obj.no_codeowners):
                                    self._relate_to_merge(branch_obj, write_ppals,
                                                          reason="Can use GitBot token to create the PR and approve it himself as codeowners are missconfigured")

                                # - If write equivalent and 1 review and codeowners but user in code owners
                                if branch_obj.require_code_owner_reviews or not known_protections:
                                    self._relate_to_merge(branch_obj, writers_codeowners,
                                                          reason="Can use GitBot token to create the PR and approve it himself (as codeowner)")

                                # - If write equivalent and 1 review and no codeowners
                                else:
                                    self._relate_to_merge(branch_obj, write_ppals,
                                                          reason="Use the GitBot token to create the PR and you approve it, or crete the PR yourself and make the GitBot approve it")
                                ...
                                self._relate_to_merge(branch_obj, write_ppals,
                                                      reason="Hijack a PR adding malicious code when it has 1 (or less) approval required")
                            ...
                            # - If admin, remove branch protections, merge code, reenable branch protections
                                ...
                                self._relate_to_merge(branch_obj, admin_ppals,
                                                      reason="Admins can remove the protection, merge, and set it again")
        ...
        if ppal_objs:
            for ppal_obj in ppal_objs:
                if type(ppal_obj) is GithubUser:
                    ...
                    self.logger.warning(
                        f"Something of type {type(ppal_obj)} arrived in _relate_to_merge. Primary value: {ppal_obj.__primarykey__}")
                    continue
